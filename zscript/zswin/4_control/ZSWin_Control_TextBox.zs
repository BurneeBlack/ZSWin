...
class ZTextBox : ZControl
{
	...
	*/
	...
	{
		bool bClipped = GetDrawingControls(txb, txb.DrawParams);
		
		if (txb.Show)
		{
			if (bClipped)
				screen.SetClipRect(txb.DrawParams.pclipX, txb.DrawParams.pclipY, txb.DrawParams.pclipWdth, txb.DrawParams.pclipHght);
			...
			switch (txb.BackgroundType)
			{
				...
				case BACKTYP_Custom:
					...
					if (txb.StretchTexture)
						screen.DrawTexture(txb.BackgroundTexture,
										txb.AnimateTexture, 
										txb.DrawParams.sxloc, 
										txb.DrawParams.syloc,
										DTA_Alpha, txb.Alpha,
						...
						*/
						...
						do
						{
							...
							do
							{
								screen.DrawTexture (txb.BackgroundTexture,
									txb.AnimateTexture,
									txb.DrawParams.sxloc + (tx * w),
									txb.DrawParams.syloc + (ty * h),
									DTA_Alpha, txb.Alpha,
									DTA_DestWidth, tx,
									DTA_DestHeight, ty);
								h++;
				...
				case BACKTYP_Color:
					screen.DrawThickLine(txb.DrawParams.sxloc,
										txb.DrawParams.syloc + (txb.Height % 2 == 0 ? txb.Height / 2 : ((txb.Height - 1) / 2) + 1),
										txb.DrawParams.sxloc + txb.Width,
										txb.DrawParams.syloc + (txb.Height % 2 == 0 ? txb.Height / 2 : ((txb.Height - 1) / 2) + 1),
										txb.Height,
										txb.BackgroundColor,
										int(255 * txb.Alpha));
			...
			// Left
			if (txb.DrawParams.pclipX > txb.DrawParams.sxloc)
			{
				bdrx = txb.DrawParams.pclipX;
				clplft = true;
			}
			else
				bdrx = txb.DrawParams.sxloc;
			
			// Top
			if (txb.DrawParams.pclipY > txb.DrawParams.syloc)
			{
				bdry = txb.DrawParams.pclipY;
				clptop = true;
			}
			else
				bdry = txb.DrawParams.syloc;
			
			// Right
			if (txb.DrawParams.pclipX + txb.DrawParams.pclipWdth < txb.DrawParams.sxloc)
				bdrwdth = 0;
			else if (txb.DrawParams.pclipX + txb.DrawParams.pclipWdth < txb.DrawParams.sxloc + txb.Width)
			{
				bdrwdth = (txb.DrawParams.pclipX + txb.DrawParams.pclipWdth) - txb.DrawParams.sxloc;
				clprht = true;
			}
			else
				bdrwdth = txb.Width;
			...
			// Bottom
			if (txb.DrawParams.pclipY + txb.DrawParams.pclipHght < txb.DrawParams.syloc)
				bdrhght = 0;
			else if (txb.DrawParams.pclipY + txb.DrawParams.pclipHght < txb.DrawParams.syloc + txb.Height)
			{
				bdrhght = (txb.DrawParams.pclipY + txb.DrawParams.pclipHght) - txb.DrawParams.syloc;
				clpbot = true;
			}
			else
				bdrhght = txb.Height;
